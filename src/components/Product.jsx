...
const Product = ( {img, title, reviews, discountedPrice, mainPrice} ) => {
    // {toString(Number(mainPrice) - Number(discountedPrice))}
  return (
    <>
<Box border="1px solid gray" h="500px" w="350px" p="10px">
<Image src={img}
p="20px" pl="20px" h="40%"></Image>
    <Link fontSize="xs" color="blue" fontWeight="10px">{title}</Link>
    <Text color="blue" mt="20px"><StarIcon color="#ffe000"/>
    ...
    <Text></Text>
    <Button mt="50px" size="sm" varient="solid" colorScheme='yellow' bg="#ffe000" leftIcon={<Icon as={AiOutlineShoppingCart} w={30} h={30} pt="0.2rem"/>}>Add to Cart</Button>
</Box>
    </>
  )
}
